...
class Splitflap(object):
    ...
    def is_in_alphabet(self, letter):
        #print("checking if %s is in alphabet with function is_in_alphabet. Result is %s"%(letter,letter in _ALPHABET))
    ...
    def set_text(self, text):
        ...
        #print("Set SplitFLapDisplay To %s with function set_text" %(text))
    ...
    def get_status(self):
        print("Asked for status with function get_status. Status is %s" %(self.last_status))
        return self.last_status


@contextmanager
def splitflap(serial_port):
    with serial.Serial(serial_port, 38400, timeout=2) as ser:
        s = Splitflap(ser)
        s._loop_for_status()
        yield s